...
PPM_PATH = r"D:\Signatures_OS\build\phase9_bofs_dashboard.ppm"
PNG_PATH = r"D:\Signatures_OS\build\phase9_bofs_dashboard.png"
ARTIFACT_PNG = r"C:\Users\Saumya Chaudhari\.gemini\antigravity-cli\brain\5b02d382-573f-440e-bff9-862ee4a7c0f9\phase9_bofs_dashboard.png"
SERIAL_LOG = r"D:\Signatures_OS\build\phase9_bofs_serial.log"
...
cmd = [
    ...
    "-m", "1024M",
    "-display", "none",
    "-monitor", "telnet:127.0.0.1:4453,server,nowait",
    "-no-reboot"
]

print("[TEST] Launching QEMU pure UEFI with BOFS Phase 9 VFS & Syscall Dashboard...")
proc = subprocess.Popen(cmd)

print("[TEST] Monitoring serial log for BOFS Phase 9 completion...")
    ...
    if os.path.exists(SERIAL_LOG):
        try:
            with open(SERIAL_LOG, "r", errors="ignore") as f:
                log_data = f.read()
                if "BOFS PHASE 9: CERTIFIED PASS [REAL-HARDWARE INTEGRATION PASS]" in log_data:
                    print(f"[TEST] BOFS Phase 9 completed successfully after {time.time() - start_time:.1f}s!")
                    test_completed = True
                    break
        except Exception:
            pass
...
try:
    print("[TEST] Connecting to QEMU monitor on port 4453...")
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect(("127.0.0.1", 4453))
    time.sleep(1.0)
...
if os.path.exists(SERIAL_LOG):
    ...
    if "BOFS PHASE 9:" in content and "REAL-HARDWARE INTEGRATION PASS" in content:
