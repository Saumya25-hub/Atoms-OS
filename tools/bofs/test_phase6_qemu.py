...
PPM_PATH = r"D:\Signatures_OS\build\phase6_bofs_dashboard.ppm"
PNG_PATH = r"D:\Signatures_OS\build\phase6_bofs_dashboard.png"
ARTIFACT_PNG = r"C:\Users\Saumya Chaudhari\.gemini\antigravity-cli\brain\5b02d382-573f-440e-bff9-862ee4a7c0f9\phase6_bofs_dashboard.png"
SERIAL_LOG = r"D:\Signatures_OS\build\phase6_bofs_serial.log"
...
cmd = [
    ...
    "-m", "1024M",
    "-display", "none",
    "-monitor", "telnet:127.0.0.1:4450,server,nowait",
    "-no-reboot"
]

print("[TEST] Launching QEMU pure UEFI with BOFS Phase 6 Directory Engine Dashboard...")
proc = subprocess.Popen(cmd)

print("[TEST] Monitoring serial log for BOFS Phase 6 completion...")
    ...
    if os.path.exists(SERIAL_LOG):
        try:
            with open(SERIAL_LOG, "r", errors="ignore") as f:
                log_data = f.read()
                if "REAL-HARDWARE INTEGRATION PASS" in log_data:
                    print(f"[TEST] BOFS Phase 6 completed successfully after {time.time() - start_time:.1f}s!")
                    test_completed = True
                    break
        except Exception:
            pass
...
try:
    print("[TEST] Connecting to QEMU monitor on port 4450...")
...
if os.path.exists(SERIAL_LOG):
    ...
    if "BOFS PHASE 6:" in content and "REAL-HARDWARE INTEGRATION PASS" in content:
