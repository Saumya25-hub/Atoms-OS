...
int main(void) {
    ...
    if (pframe) {
        FILE* fcb = fopen("build/frame5_bos_cb.raw", "wb");
        fwrite(pframe->data[1], 1, 320*180, fcb);
        fclose(fcb);

        FILE* fcr = fopen("build/frame5_bos_cr.raw", "wb");
        fwrite(pframe->data[2], 1, 320*180, fcr);
        fclose(fcr);
    }
