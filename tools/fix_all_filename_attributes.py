...
print("========================================================")
print(" SIGNATURES OS — $FILE_NAME FORENSIC MASTER FIX PASS")
print("========================================================")

# Valid Windows UTC Timestamp from Record 5
raw5 = vdi.read_at(mft_byte_off + 5 * file_record_size, file_record_size)
si_off5 = struct.unpack('<H', raw5[56+20:56+22])[0] + 56
valid_timestamps = raw5[si_off5:si_off5+32] # 4 x uint64
...
def build_fn_attr_bytes(parent_ref, fn_str, fn_ns, attr_id, timestamps_raw):
    ...
    # Header: Type 0x30, Length attr_len, NonRes 0, NameLen 0, NameOff 0, Flags 0, AttrID attr_id, ValLen fn_val_len, ValOff 24, IndexedFlag 1, Padding 0
    ...
    val_buf[8:40] = timestamps_raw # Copy valid timestamps
...
# ---------------------------------------------------------------------------
# FIX MFT RECORD #36 (saumya.txt -> DUAL $FILE_NAME: SAUMYA~1.TXT + saumya.txt)
# ---------------------------------------------------------------------------
print("Rebuilding MFT Record #36 with Dual $FILE_NAME (DOS + Win32) & IndexedFlag=1...")
saumya_content = (
    "I am Saumya.\r\n"
...
# Attr 2: $FILE_NAME (0x30, Namespace 2 DOS)
...
# Attr 3: $FILE_NAME (0x30, Namespace 1 Win32)
...
rec36[curr_off+8] = 0
rec36[curr_off+14:curr_off+16] = struct.pack('<H', 4) # Attr ID = 4
...
print("-> Record #36 Updated Cleanly")

# ---------------------------------------------------------------------------
# FIX MFT RECORD #176 (ATOMSTESTS.TXT -> DUAL $FILE_NAME: ATOMST~1.TXT + ATOMSTESTS.TXT)
# ---------------------------------------------------------------------------
print("Rebuilding MFT Record #176 (ATOMSTESTS.TXT) with Dual $FILE_NAME & IndexedFlag=1...")
atoms_content = (
    "ATOMS OS\r\n"
...
rec176[curr_off+8] = 0
rec176[curr_off+14:curr_off+16] = struct.pack('<H', 4) # Attr ID = 4
...
print("-> Record #176 Updated Cleanly")

# ---------------------------------------------------------------------------
# REBUILD & SORT ROOT DIRECTORY INDEX $I30 BLOCK (LCN 522159) WITH DOS ALIASES
# ---------------------------------------------------------------------------
print("Rebuilding and sorting Root Directory Index ($I30) with short and long names...")
indx_lcn = 522159
...
while e_off + 16 <= entries_off + total_len:
    ...
    if content_len >= 66:
        ...
        # Filter out old entries for 36 and 176
...
def create_index_entry_buf(ref, fn_str, fn_ns, file_sz):
    ...
    elen = (16 + clen + 7) & ~7
    ebuf = bytearray(elen)
    struct.pack_into('<QHHH', ebuf, 0, ref, elen, clen, 0)
...
if chk_result:
    print("\nMaster $FILE_NAME forensic fix pass completed with 100% SUCCESS!")
else:
    print("\nMaster $FILE_NAME forensic fix pass encountered errors!")
