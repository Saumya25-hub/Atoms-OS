...
cmd.exe /c "`"C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\VC\Auxiliary\Build\vcvars64.bat`" && clang -O2 tools\gpt_image_builder.c -o build\gpt_image_builder.exe"
if ($LASTEXITCODE -ne 0) { Write-Host "Compilation of gpt_image_builder failed!" -ForegroundColor Red; exit $LASTEXITCODE }

# 2.5 Ensure certified Baseline H.264 video and Dolby streams are present
if (Test-Path -LiteralPath "TEST-VIDEO\test.mp4") {
    Copy-Item -LiteralPath "TEST-VIDEO\test.mp4" -Destination "build\TEST.MP4" -Force
}
if (Test-Path -LiteralPath "TEST1[TEMP]\Dolby_Vision_AtmosHDR.mp4") {
    Copy-Item -LiteralPath "TEST1[TEMP]\Dolby_Vision_AtmosHDR.mp4" -Destination "build\DOLBY.MP4" -Force
}
if (Test-Path -LiteralPath "TEST1[TEMP]\NCSJanjiHeroesTonight.mp3") {
    Copy-Item -LiteralPath "TEST1[TEMP]\NCSJanjiHeroesTonight.mp3" -Destination "build\HEROES.MP3" -Force
    Copy-Item -LiteralPath "TEST1[TEMP]\NCSJanjiHeroesTonight.mp3" -Destination "build\TEST.MP3" -Force
}

# 3. Generate atoms_uefi_test.img & media.img
Write-Host "[3/4] Generating GPT test image: build\atoms_uefi_test.img and build\media.img..." -ForegroundColor Yellow
& .\build\gpt_image_builder.exe build\BOOTX64.EFI build\kernel.bin build\media.img
