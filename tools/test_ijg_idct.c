...
void ijg_idct_islow(const int32_t* in_block, uint8_t* out_plane, uint32_t stride) {
    ...
    for (int ctr = 0; ctr < 8; ctr++, wsptr += 8) {
        ...
        out_plane[ctr * stride + 0] = CLAMP(DESCALE(tmp0 + z4 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
        out_plane[ctr * stride + 1] = CLAMP(DESCALE(tmp1 + z3 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
        out_plane[ctr * stride + 2] = CLAMP(DESCALE(tmp2 + z2 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
        out_plane[ctr * stride + 3] = CLAMP(DESCALE(tmp3 + z1 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
        out_plane[ctr * stride + 4] = CLAMP(DESCALE(tmp3 - z1 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
        out_plane[ctr * stride + 5] = CLAMP(DESCALE(tmp2 - z2 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
        out_plane[ctr * stride + 6] = CLAMP(DESCALE(tmp1 - z3 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
        out_plane[ctr * stride + 7] = CLAMP(DESCALE(tmp0 - z4 + (128 << (CONST_BITS+PASS1_BITS+3)), CONST_BITS+PASS1_BITS+3));
    }
}
...
int main(void) {
    ...
    printf("=== IJG REFERENCE DECODED PIXELS FOR BLOCK (320, 184) ===\n");
    for (int r = 0; r < 8; r++) {
