...
BOImage *BOImage_LoadBMP(const uint8_t *data, uint32_t data_size) {
  ...
  if (info->compression != 0)
    return NULL; // Only support uncompressed BMP in v1
...
BOImage *BOImage_LoadPNG(const uint8_t *data, uint32_t data_size) {
  ...
  if (g_png_decoder_hook(data, data_size, &decoded_pixels, &width, &height) !=
          0 ||
      !decoded_pixels) {
    return NULL;
  }
...
void BOImage_ReleaseImage(int id) {
  ...
  if (handle && handle->ref_count > 0) {
    ...
    if (handle->ref_count == 0) {
      if (handle->image) {
        if (handle->image->pixels)
          kfree(handle->image->pixels);
        kfree(handle->image);
      }
      handle->id = 0;
      handle->image = NULL;
...
void BOImage_BlitToFramebuffer(int id, int32_t x, int32_t y) {
  BOImageHandle *handle = BOImage_GetImage(id);
  if (!handle || !handle->image || !handle->image->pixels)
    return;
  ...
  for (uint32_t row = 0; row < img->height; row++) {
    for (uint32_t col = 0; col < img->width; col++) {
      ...
      if (a > 0) {
        if (BWE_RenderContext_CheckClip(x + col, y + row))
  ...
  for (uint32_t row = 0; row < img->height; row++) {
    for (uint32_t col = 0; col < img->width; col++) {
      ...
      if (src_a == 255) {
        ...
        if (BWE_RenderContext_CheckClip(x + col, y + row))
          BOVISUAL_Graphics_PutPixel(x + col, y + row,
                                     (0xFF << 24) | (out_r << 16) |
                                         (out_g << 8) | out_b);
      }
    }
...
BOTexture *BOImage_CreateTexture(uint32_t width, uint32_t height,
                                 uint32_t format) {
  if (width == 0 || height == 0 || width > 4096 || height > 4096)
    return NULL;
...
void BOImage_InitExternalTexture(BOTexture *tex, uint32_t width,
...
void BOImage_DestroyTexture(BOTexture *tex) {
  if (!tex)
    return;
  if (tex->owns_data && tex->data)
    kfree(tex->data);
  if (tex->owns_object)
    kfree(tex);
}

static inline uint32_t boimage_texture_pixel(const BOTexture *tex,
                                             uint32_t index) {
  if (tex->format == 2) {
    return ((uint32_t)tex->data[index] << 24) | 0x00FFFFFFu;
  }
  return ((const uint32_t *)tex->data)[index];
}

BOAtlas *BOImage_CreateAtlas(int32_t cell_size, int32_t width_cells,
  ...
  atlas->occupancy_map =
      (uint8_t *)kcalloc((uint32_t)(width_cells * height_cells), 1);
...
void BOImage_DestroyAtlas(BOAtlas *atlas) {
  if (!atlas)
    return;
  if (atlas->occupancy_map)
    kfree(atlas->occupancy_map);
  if (atlas->atlas_texture)
    BOImage_DestroyTexture(atlas->atlas_texture);
  kfree(atlas);
}

bool BOImage_AtlasInsert(BOAtlas *atlas, BOImage *img, float *out_u1,
                         float *out_v1, float *out_u2, float *out_v2) {
  if (!atlas || !img || !img->pixels)
    return false;

  int32_t req_w =
      (int32_t)((img->width + atlas->cell_size - 1) / atlas->cell_size);
  int32_t req_h =
      (int32_t)((img->height + atlas->cell_size - 1) / atlas->cell_size);

  if (req_w > atlas->width_cells || req_h > atlas->height_cells)
    return false;
    ...
    for (int32_t cx = 0; cx <= atlas->width_cells - req_w; cx++) {
      ...
      for (int32_t dy = 0; dy < req_h; dy++) {
        for (int32_t dx = 0; dx < req_w; dx++) {
          if (atlas->occupancy_map[(cy + dy) * atlas->width_cells +
                                   (cx + dx)] != 0) {
            fits = false;
            break;
          }
      ...
      if (fits) {
        ...
        for (int32_t dy = 0; dy < req_h; dy++) {
          for (int32_t dx = 0; dx < req_w; dx++) {
            atlas->occupancy_map[(cy + dy) * atlas->width_cells + (cx + dx)] =
                1;
          }
        }
        ...
        for (uint32_t r = 0; r < img->height; r++) {
          for (uint32_t c = 0; c < img->width; c++) {
            dest_buf[(start_py + r) * atlas->atlas_texture->width +
                     (start_px + c)] = src_buf[r * img->width + c];
          }
        }
        ...
        if (out_u1)
          *out_u1 = (float)start_px / (float)atlas->atlas_texture->width;
        if (out_v1)
          *out_v1 = (float)start_py / (float)atlas->atlas_texture->height;
        if (out_u2)
          *out_u2 = (float)(start_px + img->width) /
                    (float)atlas->atlas_texture->width;
        if (out_v2)
          *out_v2 = (float)(start_py + img->height) /
                    (float)atlas->atlas_texture->height;
...
void BOImage_BatchDrawSprite(BOTexture *tex, int32_t x, int32_t y, int32_t w,
                             int32_t h, float u1, float v1, float u2,
                             float v2) {
  BOImage_BatchDrawSpriteTinted(tex, x, y, w, h, u1, v1, u2, v2, 0xFFFFFFFF);
}

void BOImage_BatchDrawSpriteTinted(BOTexture *tex, int32_t x, int32_t y,
                                   int32_t w, int32_t h, float u1, float v1,
                                   float u2, float v2, uint32_t tint_color) {
...
void BOImage_FlushBatch(BOBatch *batch) {
  ...
  for (int32_t i = 0; i < batch->count; i++) {
    ...
    if (!s->texture || !s->texture->data || s->width <= 0 || s->height <= 0)
      continue;
    ...
    int32_t draw_bottom =
        sprite_bottom < clip_bottom ? sprite_bottom : clip_bottom;
    ...
    for (int32_t dy = start_dy; dy < end_dy; dy++) {
      ...
      for (int32_t dx = start_dx; dx < end_dx; dx++) {
        ...
        uint32_t color =
            boimage_texture_pixel(s->texture, (uint32_t)(sy * tex_w + sx));
        uint8_t a = (color >> 24) & 0xFF;
        if (a > 0) {
...
void BOImage_BOHeartTickFlush(void) { BOImage_FlushBatch(&s_active_batch); }

void BOImage_DrawGlyphSpriteDirect(const BVFramebuffer *target_fb,
                                   BOTexture *texture, int32_t x, int32_t y,
                                   int32_t width, int32_t height, float u1,
                                   float v1, float u2, float v2,
                                   uint32_t tint_color) {
  if (!texture || !texture->data || width <= 0 || height <= 0)
    return;
  ...
  for (int32_t dy = start_dy; dy < end_dy; dy++) {
    ...
    for (int32_t dx = start_dx; dx < end_dx; dx++) {
      ...
      uint32_t color =
          boimage_texture_pixel(texture, (uint32_t)(sy * tex_w + sx));
      uint8_t a = (color >> 24) & 0xFF;
      if (a == 0)
        continue;
      ...
      if (px >= 0 && px < (int32_t)fb->width && py >= 0 &&
          py < (int32_t)fb->height) {
        if (a == 255) {
...
uint32_t BOImage_SamplePixel(const BOTexture *tex, float u, float v,
                             BOImageScalingFilter filter) {
  if (!tex || !tex->data)
    return 0;
  if (u < 0.0f)
    u = 0.0f;
  ...
  if (filter == BO_FILTER_NEAREST) {
    ...
    if (ty >= h)
      ty = h - 1;
    return boimage_texture_pixel(tex, ty * w + tx);
  }
  ...
  uint32_t c00 = boimage_texture_pixel(tex, y0 * w + x0);
  uint32_t c10 = boimage_texture_pixel(tex, y0 * w + x1);
  uint32_t c01 = boimage_texture_pixel(tex, y1 * w + x0);
  uint32_t c11 = boimage_texture_pixel(tex, y1 * w + x1);

  uint32_t a00 = (c00 >> 24) & 0xFF, r00 = (c00 >> 16) & 0xFF,
           g00 = (c00 >> 8) & 0xFF, b00 = c00 & 0xFF;
  uint32_t a10 = (c10 >> 24) & 0xFF, r10 = (c10 >> 16) & 0xFF,
           g10 = (c10 >> 8) & 0xFF, b10 = c10 & 0xFF;
  uint32_t a01 = (c01 >> 24) & 0xFF, r01 = (c01 >> 16) & 0xFF,
           g01 = (c01 >> 8) & 0xFF, b01 = c01 & 0xFF;
...
void BOImage_SnapBounds(float x, float y, float w, float h, int32_t *out_x,
                        int32_t *out_y, int32_t *out_w, int32_t *out_h) {
  if (out_x)
    *out_x = (int32_t)(x >= 0.0f ? x + 0.5f : x - 0.5f);
  if (out_y)
    *out_y = (int32_t)(y >= 0.0f ? y + 0.5f : y - 0.5f);
  if (out_w)
    *out_w = (int32_t)(w >= 0.0f ? w + 0.5f : w - 0.5f);
  if (out_h)
    *out_h = (int32_t)(h >= 0.0f ? h + 0.5f : h - 0.5f);
}

// Module 4: Scaling & Raster Engine
void BOImage_AtlasDrawEx(BOTexture *tex, int32_t x, int32_t y, int32_t w,
                         int32_t h, float u1, float v1, float u2, float v2,
                         BOImageScalingFilter filter) {
  if (!tex || !tex->data || w <= 0 || h <= 0)
    return;

  int32_t sx, sy, sw, sh;
  BOImage_SnapBounds((float)x, (float)y, (float)w, (float)h, &sx, &sy, &sw,
                     &sh);
  if (sw <= 0 || sh <= 0)
    return;
...
void BOImage_DrawEx(BOImage *image, int32_t x, int32_t y, int32_t width,
                    int32_t height, BOImageScalingFilter filter) {
  ...
  BOImage_AtlasDrawEx(&wrapper, x, y, width, height, 0.0f, 0.0f, 1.0f, 1.0f,
                      filter);
}
...
void BOImage_v2_RunDemo(int32_t screen_x, int32_t screen_y) {
  ...
  // 2. Showcase BOIMAGE v2.5 Quality Engine: Draw 64x64 ATOMS logo scaled up to
  // 128x128 with Bilinear Filtering!
  BOAssetHandle *logo_handle = BOAsset_Get(ASSET_LOGO);
  if (logo_handle && logo_handle->image_data) {
    BOImage_DrawEx(logo_handle->image_data, screen_x, screen_y + 48, 128, 128,
                   BO_FILTER_BILINEAR);
  }
}
