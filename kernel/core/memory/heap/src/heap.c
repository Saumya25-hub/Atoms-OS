...
void heap_audit_metadata_write(uint64_t target_addr, uint64_t old_val,
  ...
  if (target_addr < HEAP_START_VADDR || target_addr >= (uint64_t)heap_end)
    return;
  ...
  bool illegal = false;
  if (is_magic && new_val != HEAP_MAGIC)
    illegal = true;
  if (is_size && new_val > (heap_end - HEAP_START_VADDR))
    illegal = true;
  if (is_next && new_val != 0 &&
      (new_val < HEAP_START_VADDR || new_val >= (uint64_t)heap_end ||
       (new_val & 7) != 0))
    illegal = true;
  if (is_prev && new_val != 0 &&
      (new_val < HEAP_START_VADDR || new_val >= (uint64_t)heap_end ||
       (new_val & 7) != 0))
    illegal = true;
  ...
  if (illegal) {
    ...
    display_print(field);
    display_print("\n");
    display_print("Caller   : ");
    display_print(caller);
    display_print("\n");
    display_print("RIP      : ");
    display_print_hex(rip);
    display_print("\n");
    display_print("CPU ID   : ");
    display_print_dec(get_cpu_id());
    display_print("\n");
    display_print("Target   : ");
    display_print_hex(target_addr);
    display_print("\n");
    display_print("Old Value: 0x");
    display_print_hex(old_val);
    display_print("\n");
    display_print("New Value: 0x");
    display_print_hex(new_val);
    display_print("\n");
    display_print("=========================================\n");
    while (1) {
...
void heap_check_external_write(uint64_t dst_addr, size_t len,
                               const char *caller, uint64_t rip) {
  if (dst_addr < HEAP_START_VADDR || dst_addr >= (uint64_t)heap_end)
    return;

  uint64_t start = dst_addr & ~3ULL;
  for (uint64_t a = start; a < dst_addr + len && a + 4 <= (uint64_t)heap_end;
       a += 4) {
    uint32_t val = *(uint32_t *)a;
    if (val == HEAP_MAGIC) {
      heap_audit_metadata_write(a, val, 0xFF000000, "magic(EXTERNAL)", caller,
                                rip);
    }
  }
}

#define WRITE_MAGIC(blk, val)                                                  \
  do {                                                                         \
    heap_audit_metadata_write((uint64_t)&((blk)->magic), (blk)->magic, (val),  \
                              "magic", __func__,                               \
                              (uint64_t)__builtin_return_address(0));          \
    (blk)->magic = (val);                                                      \
  } while (0)

#define WRITE_SIZE(blk, val)                                                   \
  do {                                                                         \
    heap_audit_metadata_write((uint64_t)&((blk)->size), (blk)->size, (val),    \
                              "size", __func__,                                \
                              (uint64_t)__builtin_return_address(0));          \
    (blk)->size = (val);                                                       \
  } while (0)

#define WRITE_NEXT(blk, val)                                                   \
  do {                                                                         \
    heap_audit_metadata_write((uint64_t)&((blk)->next), (uint64_t)(blk)->next, \
                              (uint64_t)(val), "next", __func__,               \
                              (uint64_t)__builtin_return_address(0));          \
    (blk)->next = (val);                                                       \
  } while (0)

#define WRITE_PREV(blk, val)                                                   \
  do {                                                                         \
    heap_audit_metadata_write((uint64_t)&((blk)->prev), (uint64_t)(blk)->prev, \
                              (uint64_t)(val), "prev", __func__,               \
                              (uint64_t)__builtin_return_address(0));          \
    (blk)->prev = (val);                                                       \
  } while (0)
...
static void verify_block_canaries(heap_block_t *block, const char *caller_func,
  ...
  if (!canonical || !in_heap || !aligned) {
    ...
    display_print("==================================================\n");
    display_print("Allocation Address : 0x");
    display_print_hex(addr);
    display_print("\n");
    display_print("Allocation Size    : 0\n");
    display_print("Allocation Caller  : 0x0\n");
    display_print("Current Caller RIP : 0x");
    display_print_hex(current_rip);
    display_print("\n");
    display_print("CPU ID             : ");
    display_print_dec(get_cpu_id());
    display_print("\n");
  ...
  if (block->magic != HEAP_MAGIC) {
    ...
    display_print("==================================================\n");
    display_print("Allocation Address : 0x");
    display_print_hex(alloc_addr);
    display_print("\n");
    display_print("Allocation Size    : ");
    display_print_dec(alloc_size);
    display_print("\n");
    display_print("Allocation Caller  : 0x");
    display_print_hex(alloc_rip);
    display_print("\n");
    display_print("Current Caller RIP : 0x");
    display_print_hex(current_rip);
    display_print("\n");
  ...
  for (int i = 0; i < 8; i++) {
    if (((uint8_t *)&block->front_canary)[i] != ((uint8_t *)&exp_front)[i]) {
      ...
      display_print_dec(get_cpu_id());
      display_print("\n");
      display_print("Corrupted Offset   : -");
      display_print_dec(72 - i);
      display_print(" (Front Canary Byte ");
      display_print_dec(i);
      display_print(")\n");
      display_print("==================================================\n");
      while (1) {
  ...
  for (int i = 0; i < 8; i++) {
    if (((uint8_t *)&block->front_canary2)[i] != ((uint8_t *)&exp_front)[i]) {
      ...
      display_print("\n");
      display_print("Corrupted Offset   : -");
      display_print_dec(8 - i);
      display_print(" (Front Canary 2 Byte ");
      display_print_dec(i);
      display_print(")\n");
      display_print("==================================================\n");
      while (1) {
  ...
  if ((block->next != NULL &&
       (((uintptr_t)block->next < (uintptr_t)HEAP_START_VADDR) ||
        ((uintptr_t)block->next >= (uintptr_t)heap_end) ||
        (((uintptr_t)block->next & 7) != 0))) ||
      (block->prev != NULL &&
       (((uintptr_t)block->prev < (uintptr_t)HEAP_START_VADDR) ||
        ((uintptr_t)block->prev >= (uintptr_t)heap_end) ||
        (((uintptr_t)block->prev & 7) != 0)))) {
    unsigned long flags;
    __asm__ volatile("pushf; pop %0; cli" : "=r"(flags) : : "memory");
    display_print("\n==================================================\n");
    display_print(" [HEAP CORRUPTION DETECTED]\n");
  ...
  if (!block->is_free && alloc_size > 0) {
    ...
    for (int i = 0; i < 8; i++) {
      if (rear_ptr[i] != ((uint8_t *)&exp_rear)[i]) {
        ...
        display_print("\n");
        display_print("Corrupted Offset   : +");
        display_print_dec(alloc_size + i);
        display_print(" (Rear Canary Byte ");
        display_print_dec(i);
        display_print(")\n");
        display_print("==================================================\n");
        while (1) {
...
static bool heap_expand_locked(size_t minimum_bytes) {
  const size_t page_size = 4096;
  const size_t max_heap_bytes = 256ULL * 1024ULL * 1024ULL;
  size_t current_bytes = (size_t)(heap_end - HEAP_START_VADDR);
  if (minimum_bytes == 0 || current_bytes >= max_heap_bytes)
    return false;

  size_t grow = ALIGN_UP(minimum_bytes, page_size);
  if (grow > max_heap_bytes - current_bytes)
    grow = max_heap_bytes - current_bytes;
  grow = (grow / page_size) * page_size;
  if (grow < sizeof(heap_block_t))
    return false;
  ...
  heap_end = old_end + grow;
  heap_block_t *last = heap_head;
  while (last && last->next)
    last = last->next;

  if (last && last->is_free &&
      (uint8_t *)last + sizeof(heap_block_t) + last->size ==
          (uint8_t *)old_end) {
    WRITE_SIZE(last, last->size + grow);
  } else {
    heap_block_t *block = (heap_block_t *)old_end;
    block->front_canary = HEAP_CANARY_FRONT;
    block->front_canary2 = HEAP_CANARY_FRONT;
    block->req_size = 0;
    block->alloc_rip = 0;
    WRITE_MAGIC(block, HEAP_MAGIC);
    WRITE_SIZE(block, grow - sizeof(heap_block_t));
    block->is_free = true;
    WRITE_NEXT(block, NULL);
    WRITE_PREV(block, last);
    if (last)
      WRITE_NEXT(last, block);
    else
      heap_head = block;
  }
  return true;
...
bool heap_expand_raw(size_t minimum_bytes) {
  ...
  bool expanded = heap_expand_locked(minimum_bytes);
  heap_unlock(flags);
  return expanded;
}

void *heap_alloc_raw(size_t size, uint64_t alloc_rip) {
  if (size == 0 || size > (size_t)-1 - 15)
    return NULL;
  ...
  while (current) {
    ...
    if (current->is_free && current->size >= needed_payload) {
      ...
      if (heap_trace_enabled) {
        display_print("kmalloc(");
        display_print_dec(size);
        display_print(") -> Block ");
        display_print_hex((uint64_t)current);
        display_print("\n");
      }
  ...
  if (heap_expand_raw(needed_payload + sizeof(heap_block_t)))
    return heap_alloc_raw(size, alloc_rip);

  bmle_dump_telemetry();
  display_print("[HEAP V1] Out of Memory: failed to allocate ");
  display_print_dec(size);
  display_print(" bytes; returning NULL.\n");
  return NULL;
}
...
void *kmalloc_tracked(size_t size, uint64_t alloc_rip) {
  extern void *amsss_alloc(size_t, size_t, uint64_t);
  return amsss_alloc(size, 0, alloc_rip);
}

void *kcalloc_tracked(size_t num, size_t size, uint64_t alloc_rip) {
  extern void *amsss_calloc(size_t, size_t, uint64_t);
  return amsss_calloc(num, size, alloc_rip);
}

void *heap_realloc_raw(void *ptr, size_t new_size, uint64_t alloc_rip) {
  if (!ptr)
    return heap_alloc_raw(new_size, alloc_rip);
  if (new_size == 0) {
    heap_free_raw(ptr);
    return NULL;
  }
  if (new_size > (size_t)-1 - 15)
    return NULL;

  size_t needed_payload = ((new_size + 8) + 7) & ~7ULL;
  unsigned long flags = heap_lock();
  heap_block_t *block = (heap_block_t *)((uint8_t *)ptr - sizeof(heap_block_t));
  validate_block_or_panic(block, "krealloc");
  size_t old_size = block->req_size;

  if (block->size >= needed_payload) {
  ...
  void *new_ptr = heap_alloc_raw(new_size, alloc_rip);
  if (new_ptr) {
...
void *krealloc_tracked(void *ptr, size_t new_size, uint64_t alloc_rip) {
  extern void *amsss_realloc(void *, size_t, uint64_t);
  return amsss_realloc(ptr, new_size, alloc_rip);
}
...
void bmle_dump_telemetry(void) {
  ...
  display_print("LargeCurrentBytes=");
  display_print_dec((uint32_t)g_bmle_current_large_bytes);
  display_print("\n");
  display_print("LargePeakBytes=");
  display_print_dec((uint32_t)g_bmle_peak_large_bytes);
  display_print("\n");
  display_print("LargeAllocations=");
  display_print_dec(g_bmle_total_large_allocations);
  display_print("\n");
  display_print("LargeFrees=");
  display_print_dec(g_bmle_total_large_frees);
  display_print("\n");
  display_print("LiveLargeAllocations=");
  display_print_dec(g_bmle_live_large_allocation_count);
  display_print("\n");
  ...
  for (int r = 0; r < BMLE_MAX_RECORDS; r++) {
    if (g_bmle_records[r].active) {
      display_print(" [");
      display_print_dec(r);
      display_print("] Ptr: 0x");
      display_print_hex((uint64_t)g_bmle_records[r].ptr);
      display_print(" Size: ");
      display_print_dec(g_bmle_records[r].size);
      display_print(" RIP: 0x");
      display_print_hex(g_bmle_records[r].caller_rip);
      display_print("\n");
    }
  }
  display_print("====================================\n\n");
...
void heap_dump_blocks(void) {
  ...
  while (current) {
    ...
    display_print("Block ");
    display_print_dec(block_num++);
    display_print("\n");
    display_print("Address : ");
    display_print_hex((uint64_t)current);
    display_print("\n");
    display_print("Size    : ");
    display_print_dec(current->size);
    display_print(" bytes\n");
    display_print("State   : ");
    if (current->is_free)
      display_print("FREE\n\n");
...
void heap_stress_test(void) {
  ...
  display_print("Initial Free: ");
  display_print_dec(initial_stats.free_size);
  display_print("\n");
  ...
  display_print("Allocated 500 blocks. Success: ");
  display_print_dec(alloc_count);
  display_print("\n");
  ...
  display_print("Final Free:   ");
  display_print_dec(final_stats.free_size);
  display_print("\n");
...
void heap_walk(void) {
  ...
  while (current) {
    ...
    if (current->is_free)
      ...
      display_print("USED\n");
    display_print("Prev    : ");
    display_print_hex((uint64_t)current->prev);
    display_print("\n");
    display_print("Current : ");
    display_print_hex((uint64_t)current);
    display_print("\n");
    display_print("Next    : ");
    display_print_hex((uint64_t)current->next);
    display_print("\n\n");
...
void *kmalloc_aligned_tracked(size_t size, size_t alignment,
                              uint64_t alloc_rip) {
  extern void *amsss_alloc(size_t, size_t, uint64_t);
  return amsss_alloc(size, alignment, alloc_rip);
}
