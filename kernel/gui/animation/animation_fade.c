...
static void wallpaper_fade_update(uint32_t anim_id, int32_t current_val,
                                  void *user_data) {
  (void)anim_id;
  (void)user_data;
...
void wallpaper_transition(struct BOSSurface *new_wallpaper,
  ...
  if (g_wallpaper_anim_id > 0) {
    ...
    g_wallpaper_anim_id = 0;
    /* Explicit cancellation has no completion callback. Commit the
       previous transition's retirement before replacing it. */
    desktop_end_wallpaper_transition();
  }
  ...
  // start 0 to 1000
  uint32_t anim_id = animation_start(ANIM_TYPE_WALLPAPER_TRANSITION, 0, 1000,
                                     duration_ms, EASING_EASE_IN_OUT);
  ...
  if (anim_id > 0) {
    ...
    // Fallback if max animations reached: commit without a fade.
    desktop_end_wallpaper_transition();
    desktop_set_wallpaper(new_wallpaper);
    desktop_refresh_background();
  }
