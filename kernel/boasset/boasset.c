...
BOAssetHandle* BOAsset_Load(uint32_t asset_id, const char* filepath, BOAssetType type) {
    ...
    // Ensure master atlas exists (e.g. 64x64 cell grid, 4x4 cells = 256x256)
    if (!s_master_atlas) {
        s_master_atlas = BOImage_CreateAtlas(64, 4, 4);
    }
...
void BOAsset_PreloadCritical(void) {
    // Preload critical icons so they live permanently inside BOIMAGE v2 atlas
