...
struct BOSSurface* wallpaper_scaler_scale(struct BOSSurface* src, int target_w, int target_h, WallpaperScaleMode mode, uint32_t bg_color) {
    ...
    if (mode == WALLPAPER_SCALE_STRETCH || mode == WALLPAPER_SCALE_FIT || mode == WALLPAPER_SCALE_FILL) {
        int* x_map = (int*)kmalloc(target_w * sizeof(int));
        if (x_map) {
        ...
        for (int y = 0; y < src_h; y++) {
            int dest_y = y + dy;
            if (dest_y < 0 || dest_y >= target_h) continue;
            ...
            for (int x = 0; x < src_w; x++) {
                int dest_x = x + dx;
                if (dest_x < 0 || dest_x >= target_w) continue;
