...
BOSImage *bopawn_load(const char *path) {
  ...
  switch (format) {
  case BOPAWN_FORMAT_PNG:
    surface = png_decode(buffer, file_size);
    break;
  case BOPAWN_FORMAT_BMP:
    surface = bmp_decode(buffer, file_size);
    break;
  case BOPAWN_FORMAT_ICO:
    surface = ico_decode(buffer, file_size);
    break;
  case BOPAWN_FORMAT_RAW:
    surface = raw_decode(buffer, file_size);
    break;
  default:
    break;
  }
...
void bopawn_destroy(BOSImage *image) {
  ...
  if (image)
    image_cache_release(image);
}

void bopawn_preload(const char *path) { bopawn_load(path); }
