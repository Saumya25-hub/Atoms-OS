...
int aes_set_key(AES_CTX* ctx, const uint8_t* key, size_t key_len) {
    ...
    int nk = 0;
    if (key_len == 16) { nk = 4; ctx->rounds = 10; }
    else if (key_len == 32) { nk = 8; ctx->rounds = 14; }
    else return -1;
    ...
    int total_words = 4 * (ctx->rounds + 1);
...
void aes_encrypt_block(const AES_CTX* ctx, const uint8_t in[16], uint8_t out[16]) {
    ...
    for (int round = 1; round <= ctx->rounds; round++) {
        ...
        // MixColumns (only for rounds 1 to rounds-1)
        if (round < ctx->rounds) {
            for (int c = 0; c < 4; c++) {
        ...
        // AddRoundKey
        for (int c = 0; c < 4; c++) {
            uint32_t rk = ctx->round_keys[round * 4 + c];
            state[0][c] ^= (rk >> 24) & 0xFF;
            state[1][c] ^= (rk >> 16) & 0xFF;
            state[2][c] ^= (rk >> 8) & 0xFF;
            state[3][c] ^= rk & 0xFF;
