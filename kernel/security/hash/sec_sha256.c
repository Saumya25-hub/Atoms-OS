 ...
 * sec_sha256.c — FIPS 180-4 Standard SHA-256 Implementation
 */
...
static void sha256_transform(bos_sha256_ctx_t* ctx, const uint8_t data[64]) {
...
void bos_sha256_update(bos_sha256_ctx_t* ctx, const void* data, size_t len) {
    ...
    for (size_t i = 0; i < len; ++i) {
        ctx->buffer[ctx->count % 64] = bytes[i];
        ctx->count++;
        if (ctx->count % 64 == 0) {
...
bos_sec_status_t bos_sha256(const void* data, size_t len, uint8_t hash[32]) {
    ...
    bos_sha256_init(&ctx);
    bos_sha256_update(&ctx, data, len);
    bos_sha256_final(&ctx, hash);
    return BOS_SEC_OK;
}
