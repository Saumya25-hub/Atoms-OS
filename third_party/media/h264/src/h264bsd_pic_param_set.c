...
/* Code */
    ...
    if (tmp == END_OF_STREAM)
        return(HANTRO_NOK);
    pPicParamSet->entropyCodingModeFlag = (tmp == 1) ? HANTRO_TRUE : HANTRO_FALSE;
    ...
    /* High Profile PPS extension (ISO/IEC 14496-10 Section 7.3.2.2) */
    if (h264bsdMoreRbspData(pStrmData))
    {
        tmp = h264bsdGetBits(pStrmData, 1);
        if (tmp == END_OF_STREAM)
            return(HANTRO_NOK);
        pPicParamSet->transform8x8ModeFlag = (tmp == 1) ? HANTRO_TRUE : HANTRO_FALSE;

        tmp = h264bsdGetBits(pStrmData, 1);
        if (tmp == END_OF_STREAM)
            return(HANTRO_NOK);
        pPicParamSet->picScalingMatrixPresentFlag = (tmp == 1) ? HANTRO_TRUE : HANTRO_FALSE;

        if (pPicParamSet->picScalingMatrixPresentFlag)
        {
            u32 n_lists = 6 + ((pPicParamSet->transform8x8ModeFlag) ? 2 : 0);
            for (i = 0; i < n_lists; i++)
            {
                tmp = h264bsdGetBits(pStrmData, 1);
                if (tmp)
                {
                    ...
                    u32 sizeOfScalingList = (i < 6) ? 16 : 64;
                    for (u32 j = 0; j < sizeOfScalingList; j++)
                    {
                        if (nextScale != 0)
                        {
                            i32 deltaScale;
                            h264bsdDecodeExpGolombSigned(pStrmData, &deltaScale);
        ...
        tmp = h264bsdDecodeExpGolombSigned(pStrmData, &itmp);
        if (tmp == HANTRO_OK)
            pPicParamSet->secondChromaQpIndexOffset = itmp;
        else
            pPicParamSet->secondChromaQpIndexOffset = pPicParamSet->chromaQpIndexOffset;
    }
    else
    {
        pPicParamSet->secondChromaQpIndexOffset = pPicParamSet->chromaQpIndexOffset;
