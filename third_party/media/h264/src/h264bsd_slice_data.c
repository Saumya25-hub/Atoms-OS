...
/* Code */
    ...
    CabacDecCtx cabacCtx;
    CabacContextTable cabacTbl;
    if (pStorage->activePps->entropyCodingModeFlag)
    {
        if (h264bsdInitCabac(&cabacCtx, pStrmData) != HANTRO_OK)
            return(HANTRO_NOK);
        h264bsdInitCabacContexts(&cabacTbl, pSliceHeader->sliceType, qpY, pSliceHeader->cabacInitIdc);
    }
    ...
    do
    {
        ...
        u32 cabacEndOfSlice = HANTRO_FALSE;
        if (pStorage->activePps->entropyCodingModeFlag)
        {
            tmp = h264bsdDecodeCabacMacroblockLayer(&cabacCtx, &cabacTbl,
                mbLayer, pStorage->mb + currMbAddr, pSliceHeader->sliceType,
                pSliceHeader->numRefIdxL0Active);
            if (tmp == 2)
            {
                cabacEndOfSlice = HANTRO_TRUE;
                tmp = HANTRO_OK;
            }
            if (tmp != HANTRO_OK)
            {
                EPRINT("cabac_macroblock_layer");
                return(tmp);
            }
        }
        else
        ...
        if (pStorage->activePps->entropyCodingModeFlag)
        {
            moreMbs = (!cabacEndOfSlice && (currMbAddr + 1 < pStorage->picSizeInMbs)) ?
