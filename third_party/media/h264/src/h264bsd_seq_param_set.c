...
/* Code */
    ...
    /* High Profile extension syntax (ISO/IEC 14496-10 Section 7.3.2.1.1) */
    pSeqParamSet->chromaFormatIdc = 1; /* Default 4:2:0 */
    pSeqParamSet->bitDepthLuma = 8;
    pSeqParamSet->bitDepthChroma = 8;
    if (pSeqParamSet->profileIdc == 100 || pSeqParamSet->profileIdc == 110 ||
        pSeqParamSet->profileIdc == 122 || pSeqParamSet->profileIdc == 244 ||
        pSeqParamSet->profileIdc == 44  || pSeqParamSet->profileIdc == 83  ||
        pSeqParamSet->profileIdc == 86  || pSeqParamSet->profileIdc == 118 ||
        pSeqParamSet->profileIdc == 128 || pSeqParamSet->profileIdc == 138 ||
        pSeqParamSet->profileIdc == 139 || pSeqParamSet->profileIdc == 134 ||
        pSeqParamSet->profileIdc == 135)
    {
        tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &value);
        if (tmp != HANTRO_OK) return(tmp);
        pSeqParamSet->chromaFormatIdc = value;
        if (pSeqParamSet->chromaFormatIdc == 3)
        {
            tmp = h264bsdGetBits(pStrmData, 1);
            if (tmp == END_OF_STREAM) return(HANTRO_NOK);
            pSeqParamSet->separateColourPlaneFlag = tmp;
        }
        tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &value);
        if (tmp != HANTRO_OK) return(tmp);
        pSeqParamSet->bitDepthLuma = value + 8;
        tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &value);
        if (tmp != HANTRO_OK) return(tmp);
        pSeqParamSet->bitDepthChroma = value + 8;
        tmp = h264bsdGetBits(pStrmData, 1);
        if (tmp == END_OF_STREAM) return(HANTRO_NOK);
        pSeqParamSet->qpprimeYZeroTransformBypassFlag = tmp;
        tmp = h264bsdGetBits(pStrmData, 1);
        if (tmp == END_OF_STREAM) return(HANTRO_NOK);
        pSeqParamSet->seqScalingMatrixPresentFlag = tmp;
        if (pSeqParamSet->seqScalingMatrixPresentFlag)
        {
            u32 numScalingLists = (pSeqParamSet->chromaFormatIdc != 3) ? 8 : 12;
            for (u32 sl = 0; sl < numScalingLists; sl++)
            {
                tmp = h264bsdGetBits(pStrmData, 1);
                if (tmp == END_OF_STREAM) return(HANTRO_NOK);
                if (tmp)
                {
                    ...
                    u32 sizeOfScalingList = (sl < 6) ? 16 : 64;
                    for (u32 j = 0; j < sizeOfScalingList; j++)
                    {
                        if (nextScale != 0)
                        {
                            i32 deltaScale;
                            tmp = h264bsdDecodeExpGolombSigned(pStrmData, &deltaScale);
                            if (tmp != HANTRO_OK) return(tmp);
